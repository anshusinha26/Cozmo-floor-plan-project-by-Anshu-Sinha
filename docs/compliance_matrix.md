...
| Damage regions with class enum and extent | cozmo/pipeline/damage_stage.py, cozmo/damage/ | damage_regions[], DamageClass | partial: runs inside `cozmo run` at the lidar and photo tiers under --damage auto/on/off; video is skipped with a warning. Extents are measured only where there is depth. Precision is poor on photos, 24 regions in a room with 2 marks: docs/damage_eval/README.md |
| Concealed-damage flags with rule id and evidence | cozmo/contracts/models.py, cozmo/damage/rules.py | concealed_damage_flags[] | done: the rules engine emits a rule id and its evidence into plan.json from `cozmo run` |
| Scope items with quantity and basis | cozmo/contracts/models.py, cozmo/damage/pipeline.py | scope_items[] | done: generated from the surviving damage regions and written into plan.json by `cozmo run` |
